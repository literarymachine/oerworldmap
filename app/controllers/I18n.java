...
public class I18n extends OERWorldMap {
  ...
  public Result get() {
    ...
    ResourceBundle messageBundle = ResourceBundle.getBundle("messages", mLocale);
    for (String key : Collections.list(ResourceBundle.getBundle("messages", mLocale).getKeys())) {
      try {
    ...
    i18n.put("countries", Countries.map(mLocale));
    i18n.put("languages", Languages.map(mLocale));
